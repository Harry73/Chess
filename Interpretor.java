...
public class Interpretor {
	...
	public void addNextMove(Coord from, Coord to, Piece taken, String extraInfo) {
		String nextMove = "";
		
		nextMove += "" + (char)((int)(from.X() + 65)) + ((int)from.Y() + 1);
		nextMove += " " + (char)((int)to.X() + 65) + ((int)to.Y() + 1);
		...
		if (taken != null) {
			...
			nextMove += (char)((int)taken.getLocation().X() + 65) + "" + (int)(taken.getLocation().Y() + 1) + " ";
	...
	public Coord[] getLastMove() {
		...
		returnInfo[0] = new Coord((int)(fromSquare.charAt(0) - 65), Integer.parseInt(fromSquare.charAt(1) + "") - 1);
		returnInfo[1] = new Coord((int)(toSquare.charAt(0) - 65), Integer.parseInt(toSquare.charAt(1) + "") - 1);		
		...
		if (promotionCastlePart.contains("promotion") || promotionCastlePart.contains("0-0")) {
			...
			if (promotionCastlePart.contains("promotion"))
				returnInfo[4] = new Coord(4, 4);
			else if (promotionCastlePart.contains("0-0"))
				returnInfo[3] = new Coord(3, 3);
		}
	...
	public Piece getTakenPiece() {
		...
		if (type.contains("queen"))
			newPiece = new Queen("queen", color, new Coord(x, y), hasMoved);
		else if (type.contains("rook"))
			newPiece = new Rook("rook", color, new Coord(x, y), hasMoved);
		else if (type.contains("knight"))
			newPiece = new Knight("knight", color, new Coord(x, y), hasMoved);
		else if (type.contains("bishop"))
			newPiece = new Bishop("bishop", color, new Coord(x, y), hasMoved);
		else if (type.contains("pawn"))
			newPiece = new Pawn("pawn", color, new Coord(x, y), hasMoved);
